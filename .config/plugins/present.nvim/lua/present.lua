...
local function create_floating_terminal(config, enter)
  if enter == nil then
    enter = false
  end
...
local execute_lua_code = function(block)
  ...
  -- Store the original print funciton
  ...
  pcall(function()
    ...
    table.insert(output, "")
    if not chunk then
      table.insert(output, "ERROR: BROKEN CODE")
    else
      chunk()
    end
...
local create_window_configuration = function()
  ...
  local header_height = 1 + 2 -- plus border
  local footer_height = 1 -- no border
  local body_height = height - header_height - footer_height - 2 - 1 -- minus border
...
M.start_presentation = function(opts)
  ...
  state.floats.background = create_floating_terminal(windows.background)
  state.floats.header = create_floating_terminal(windows.header)
  state.floats.footer = create_floating_terminal(windows.footer)
  state.floats.body = create_floating_terminal(windows.body, true)
  ...
  present_keymap("n", "<right>", function()
    state.current_slide = math.min(state.current_slide + 1, #state.parsed.slides)
    set_slide_content(state.current_slide)
  end)

  present_keymap("n", "<left>", function()
  ...
  present_keymap("n", "X", function()
    ...
    -- TODO: Make a way for people to execute this for other language
  ...
  vim.api.nvim_create_autocmd("BufLeave", {
    buffer = state.floats.body.buf,
    callback = function()
      -- Reset the value when we are done with the presentation
      for option, config in pairs(restore) do
